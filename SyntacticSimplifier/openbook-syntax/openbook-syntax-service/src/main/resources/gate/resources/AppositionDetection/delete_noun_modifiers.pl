...
# encoding used by the TTT2
	...
	if($pc_sent =~ /aggressively se/){
#	    print STDERR "$num_quotes\t$pc_sent\t$ratio\n";exit;
	}
...
# Now processing Subordination
#
# For subordination, operating entirely in order of linear precedence, not
# assuming that lexical signs must be processed before punctuation signs.
		my $c;
...
#		    my $pc_match_pattern = "<PC ID\=\"([0-9]+)\" CLASS\=\"$pcc\">([\:\;\, abcdehilnortuw]+)<\/PC>";# matches every sign
		    my $pc_match_pattern = "<PC ID\=\"([0-9]+)\" CLASS\=\"$pcc\">(that|which|who|\,\s+and|\,\s+but|\,\s+which|\,\s+who|\,|\;)<\/PC>";
		    my $pc_match_pattern = "<PC ID\=\"([0-9]+)\" CLASS\=\"$pcc\">(that|which|who|\,\\\s+and|\,\\\s+but|\,\\\s+which|\,\\\s+who|\,|\;)<\/PC>";


#		    print STDERR "\n\ntrying to match $pc_match_pattern\nin $si\n\n";		    
...
#			print STDERR "\n\n #################### $this_pc\n\n";
			...
			if($pcc =~ /^(SSCCV|SSMV|SSMN|SSMP|SSMA)$/){
			    if($precedes_pc =~ /<w c\=\"w\" p\=\"(NN|NNS|NNP|NNPS)\">([^>]+)<\/w>(\s*)$/){
				unless($divided_once == 1){
#				    print STDERR "#################### $this_pc\n\n";
				    my $qc_nw = 10;
				    if($precedes_pc =~ />so</){
					my $quick_check = $POSTMATCH;
					$qc_nw = 0;
					while($quick_check =~ />([A-Za-z\'\_]+)</g){
					    $qc_nw++;
					}
				    }

				    if($qc_nw > 3){
					$simpler_pair_ref = TagNounModifiers::simplify($si, $precedes_sc, $follows_sc, $this_pc);
					$divided_once = 1;
					$num_calls_SSCCV++;
				    }
				}
...
#				unless($si =~ /CLASS\=\"(SSCCV|SSMN|SSMA|SSMP|SSMV|CCV|CMV1|CMN1|CMP|CMA1)\">(and|but|or|that|which|who|\,\s+and|\,\s+but|\,\s+which|\,)</i){
			    unless($si =~ /CLASS\=\"(SSCCV|SSMN|SSMA|SSMP|SSMV)\">(and|but|or|that|which|who|\,\s+and|\,\s+but|\,\s+which|\,\s+who|\,)</i){
#				unless(1 == 0){
#				print STDERR ">>>\[$si\]\n"; # exit
				push(@simplified_sentences, $si);
			    }
			    else{
				print STDERR "NOT PUSHING \[$si\] INTO FINAL OUTPUT SET\nalready pushed \= $already_pushed_this_sent\n";
			    }
			}
		    }
		}
...
# specified above
	    ...
	    if($verbosity == 1){
		print STDERR "\nFINAL SET OF SIMPLIFIED SENTENCES\n";
		my $x;
...
#		print STDERR "ORIGINAL SENTENCE: $original_sentence\n";
		...
		if($sss > 0){ # For debugging, easier to impose this condition
#		    print("\n$print_sentence\n");
...
# Deleting XML token and sign elements from output 11/03/2013
			$fps =~ s/<([^>]+)>//g;
# tokenising untokenised quotes 11/03/2013
#			$fps =~ s/([\']+)<([A-Za-z]+)/<w c\=\"lquote\" qut\=\"s\" p\=\"\$1\">$1<\/w><$2/g;
#			$fps =~ s/ \"<w / <w c\=\"lquote\" qut\=\"s\" p\=\"\'\">\'\'<\/w><w /g;
			my $raw_sentence = $fps;
			$raw_sentence =~ s/\{([^\s]+)\} //g;
...
# To avoid printing unchanged sentences, check "if($derivation =~ /RNM\-/){" in
# the line below.
				    if($derivation =~ /(RNM|XNM)\-/){
					$print_fps =~ s/\[MODIFIER\]/<MODIFIER>/g;
					$print_fps =~ s/\[\/MODIFIER\]/<\/MODIFIER>/g;
					...
					$print_fps =~ s/\s+/ /g;
#					print("<SENTENCE>\n<ORIGINAL>$print_original_sentence<\/ORIGINAL>\n<SIMPLIFIED>\n\t<S DERIVATION\=\"$derivation\">$print_fps<\/S>\n<\/SIMPLIFIED>\n<\/SENTENCE>\n");
#					print("<SENTENCE>\n<ORIGINAL>$xml_tagged_sentence<\/ORIGINAL>\n<SIMPLIFIED>\n\t<S DERIVATION\=\"$derivation\">$print_fps<\/S>\n<\/SIMPLIFIED>\n<\/SENTENCE>\n");

#					$print_fps = "<S>".$xml_tagged_sentence."</S>";
#					$print_fps =~ s/<S>\s+/<S>/;
#					$print_fps =~ s/\s+<\/S>/<\/S>/;
...
#		    $xml_tagged_sentence =~ s/([\']+)<([A-Za-z]+)/<w c\=\"lquote\" qut\=\"s\" p\=\"\$1\">$1<\/w><$2/g;
#		    $xml_tagged_sentence =~ s/ \"<w / <w c\=\"lquote\" qut\=\"s\" p\=\"\'\">\'\'<\/w><w /g;
#		    $xml_tagged_sentence = "<S>".$xml_tagged_sentence."</S>";
#		    $xml_tagged_sentence =~ s/<S>\s+/<S>/;
#		    $xml_tagged_sentence =~ s/\s+<\/S>/<\/S>/;

		    $xml_tagged_sentence =~ s/<([^>]+)>//g;

		    print("$print_original_sentence\n");
		    $sent_printed = 1;
#		    print("$xml_tagged_sentence\n");
#		    print("<SENTENCE>\n<ORIGINAL>$xml_tagged_sentence<\/ORIGINAL>\n<\/SENTENCE>\n");
#		    print("<SENTENCE>\n<ORIGINAL>$print_original_sentence<\/ORIGINAL>\n<\/SENTENCE>\n");
		}
	    }
...
#	    my $num_ss = @simplified_sentences;
#	    if($num_ss > 0){
#		print("<\/SENTENCE>\n");
#	    }
	}
