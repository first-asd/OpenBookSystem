...
my $which = "<w c\=\"w\" p\=\"WDT\">which<\/w>";
my $and = "<PC ID\=\"([0-9]+)\" CLASS\=\"([^\"]+)\">and<\/PC>";
...
my $for = "<w c\=\"w\" p\=\"IN\">for<\/w>";
my $if = "<w c\=\"w\" p\=\"IN\">if<\/w>";
my $by = "<w c\=\"w\" p\=\"IN\">by<\/w>";
my $although = "<w c\=\"w\" p\=\"IN\">although<\/w>";
...
#	die "$precedes_leftmost_subordinator";
	if($follows_leftmost_subordinator =~ /<PC ID\=\"([0-9]+)\" CLASS\=\"E([^\"]+)\">([^<]+)<\/PC>/){
		...
		if($class eq "ESCCV"){
		    if($precedes_leftmost_subordinator =~ /(($any_kind_of_noun|$any_kind_of_determiner|$and|\s)*?)($any_kind_of_noun)/){
			$subordinate_subject = $&;
			...
			if($sentence1 =~ /\Q$element_for_deletion\E/){
			    $matrix_start = $PREMATCH;
			    $matrix_end = $POSTMATCH;
			}

			$sentence1 =~ s/\Q$element_for_deletion\E//;
			$sentence2 = $subordinate_subject.$subordinated_element;
			
			($sentence1, $sentence2) = improve_elements($potential_subordinator, $terminator, $matrix_start, $matrix_end, $subordinate_subject, $subordinated_element);
...
sub improve_elements{
    my($start_sign, $end_sign, $matrix_start, $matrix_end, $subordinate_subject, $subordinated_element) = @_;
    my $sentence1;
    my $sentence2;

    my $stop_flag = 0;

    my $se_word_length = count_words($subordinated_element);
    my $detagged_start_sign = detag_sign($start_sign);
    my $detagged_end_sign = detag_sign($end_sign);

    print STDERR "MATRIX START $matrix_start\nMATRIX END $matrix_end\nSUBORDINATE SUBJECT $subordinate_subject\nSUBORDINATED ELEMENT $subordinated_element\n\n";

    if($subordinated_element =~ /($any_kind_of_pron)/){
# This seems to be quite a cause of error, but needs to be handled in WP4. Maybe set $sentence1 = $matrix_start.$subordinated_element.$matrix_end;
# May need to add the de-tagged sign before $subordinated_element below:
	$sentence1 = $matrix_start.$detagged_start_sign.$subordinated_element.$end_sign.$matrix_end;
	$sentence2 = "";
    }
    elsif($se_word_length < 3){
# This seems to be quite a cause of error, but needs to be handled in WP4. Maybe set $sentence1 = $matrix_start.$subordinated_element.$matrix_end;
# May need to add the de-tagged sign before $subordinated_element below:
	$sentence1 = $matrix_start.$detagged_start_sign.$subordinated_element.$end_sign.$matrix_end;
	$sentence2 = "";
    ...
    elsif($subordinated_element =~ /^\s*($as|$because|$although)/){
	$sentence1 = $POSTMATCH;
	$sentence2 = $matrix_start.$matrix_end;

	$stop_flag = 0;
    }
    elsif($subordinated_element =~ /^\s*($if)/){
	$sentence1 = $matrix_start.$detagged_start_sign.$subordinated_element.$end_sign.$matrix_end;
	$sentence2 = "";

	$stop_flag = 0;
    }
    elsif($subordinated_element =~ /^\s*($any_kind_of_prep)(\s*)($which)/){
	my $p = $1;
	my $ser = $POSTMATCH;
	$sentence2 = $matrix_start.$matrix_end;
	$sentence1 = $matrix_start.$ser." ".$p." ".$subordinate_subject;

	$stop_flag = 0;
    }
    elsif($subordinated_element =~ /^\s*($whose)/){
	my $rse = $POSTMATCH;
	my $possessive_subject = $subordinate_subject;
	my $ps_head = "";
	my $mps_head;

	while($possessive_subject =~ /($any_kind_of_noun)/g){
	    $ps_head = $&;
	    $mps_head = $ps_head;
	    if($ps_head =~ /\"([^\"]+)S\"/){
		$ps_head =~ s/s</s\'</;
	...
	$subordinate_subject =~ s/$mps_head/$ps_head/;

	$sentence2 = $subordinate_subject.$rse;
	$sentence1 = $matrix_start.$matrix_end;

	$stop_flag = 0;
    }
    elsif($subordinated_element =~ /^\s*($open_bracket)/){
	$sentence2 = $POSTMATCH;
	$sentence1 = $matrix_start.$matrix_end;
	$stop_flag = 0;
    }
    elsif($subordinated_element =~ /($ccv_comp_verb)(\s*)$/){
	$sentence2 = "";
	$sentence1 = $subordinated_element.$matrix_start.$matrix_end;
	$stop_flag = 0;
    }
    elsif($subordinated_element =~ /^(\s*)($ccv_comp_verb)/){
	$sentence2 = $subordinate_subject.$&.$POSTMATCH;
	$sentence1 = $matrix_start.$matrix_end;
	$stop_flag = 0;
    }
    elsif($subordinated_element =~ /^((\s|$any_kind_of_modal|$by|$any_kind_of_adverb)*)($vbd_verb|$vbz_verb|$vb_verb|$vbp_verb)/){
	$sentence2 = $subordinate_subject.$&.$POSTMATCH;
	$sentence1 = $matrix_start.$matrix_end;
	$stop_flag = 0;
    }
    ...
    $sentence1 = convert_substring_to_sentence($sentence1);
    $sentence2 = convert_substring_to_sentence($sentence2);
...
sub convert_substring_to_sentence{
    my($sentence) = @_;


    $sentence =~ s/^(\s*)<w c\=\"w\"([^>]+)>([a-z])([a-z]+)/$1<w c\=\"w\"$2>\U$3\L$4/;
    if($sentence !~ /sb\=\"ttrruuee\"/ && $sentence =~ /<w /){

	if($sentence =~ /\s*($quotes)\s*$/){
	    $sentence =~ s/(\s*)($quotes)(\s*)$/<w c\=\"\.\" sb\=\"ttrruuee\">\.<\/w>$1$2$3/;
	}
	else{
	    $sentence .= "<w c\=\"\.\" sb\=\"ttrruuee\">\.<\/w>";
	}
    }
...
sub detag_sign{
    ...
    $sign =~ s/<PC ([^>]+)>([\,\:\;])<\/PC>/<w c\=\"cm\" p\=\"$2\">$2<\/w>/;
    $sign =~ s/<PC ([^>]+)>([\,\:\;])(\s+)([^<]+)<\/PC>/<w c\=\"cm\" p\=\"$2\">$2<\/w>$3<w c\=\"w\" p\=\"DT\">$4<\/w>/;
