...
if($input_file =~ /\/([^\/]+)$/){
    ...
    $guidance_file = $guidance_dir."guidance.".$file_label;
    open(GUIDANCE, ">$guidance_file");
    print GUIDANCE "";
    close(GUIDANCE);
...
#    my $editing_guidance = EditingGuidance::guidance($xml_tagged_sentence);
    my $editing_guidance = EfficientEditingGuidance::guidance($xml_tagged_sentence);
    
    if($editing_guidance ne ""){
	open(GUIDANCE, ">>$guidance_file");
