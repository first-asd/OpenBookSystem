...
#		exit;
	    ...
	    #IUSTIN FIXME pathis inside code :(  open(PROBLEM, ">/home/richard/FIRST/WP7_TestingAndEvaluation/corpora/temp.txt");
	    open(PROBLEM, ">problem_temp.txt");
...
#			$fps =~ s/([\']+)<([A-Z]+)/<w c\=\"lquote\" qut\=\"s\" p\=\"\$1\">$1<\/w><$2/g;
			my $raw_sentence = $fps;
			$raw_sentence =~ s/\{([^\s]+)\} //g;
...
####IUSTIN uncommented next 3 lines
