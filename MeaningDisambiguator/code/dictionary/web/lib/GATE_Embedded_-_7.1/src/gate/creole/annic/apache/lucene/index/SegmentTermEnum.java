...
final class SegmentTermEnum extends TermEnum implements Cloneable {
  ...
  private final Term readTerm() throws IOException {
    ...
    if (buffer.length < length/*totalLength*/)
      growBuffer(length);

    input.readChars(buffer, 0, length);
    String text = new String(buffer, 0, length);
    /* Niraj */
    int typeLen = input.readVInt();
    if(buffer.length < typeLen)
      growBuffer(typeLen);

    input.readChars(buffer,0,typeLen);
    /*int position = input.readVInt();*/
    /* End */
    int fieldPos = input.readVInt();
    return new Term(fieldInfos.fieldName(fieldPos), text, new String(buffer,0,typeLen)/*, position*/, false);
    /*return new Term(fieldInfos.fieldName(fieldPos),
